...
export async function handleDirectCommand(parsed: ParsedCliArgs): Promise<boolean> {
  ...
  if (parsed.command === "baseline") {
    ...
    if (diff.added.length > 0) writeStdout(`Added: ${diff.added.length}\n`);
    if (diff.removed.length > 0) writeStdout(`Removed: ${diff.removed.length}\n`);
    if (diff.changed.length > 0) writeStdout(`Changed: ${diff.changed.length}\n`);
